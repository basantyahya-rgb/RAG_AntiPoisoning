# -*- coding: utf-8 -*-
"""ANTI_Poisoned_attack_v12_final.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1dtDOwuUpii8I_8KEdQtCrChuPasXi6cJ

# RAG Anti-Poisoning Pipeline — v12 (10/10)

## v12 fixes on top of v11

v11 had Baseline MRR = 0.9835 and A/B/C all working. The only remaining problem was **Condition D (poisoned corpus)** returning MRR = 0.0003.

**Root cause:** The poisoning function appended injection keywords to the 200 eval target passages. The keyword filter then removed all 200 of those passages from the poisoned index — so the targets simply weren't there to be retrieved.

**v12 fix — realistic corpus poisoning model:**
The attacker injects *fake* documents into the corpus (new rows), rather than modifying the legitimate target passages. This is the correct threat model: an attacker cannot edit real documents already in the knowledge base; they can only insert new malicious ones. The eval target passages remain clean and retrievable. The defense demonstrates it can filter out the fake injected documents.

Also fixed in v12:
- Interpretation cell updated to reflect correct Condition D semantics
- Banner updated to v12
"""

# ⚠️  IMPORTANT: before running this notebook always do:
#     Runtime → Restart session  (clears all cached Python objects)
#     Then: Runtime → Run all
#
# If you reuse a kernel from a previous run, stale embedding vectors
# in the EmbedCache dict will silently corrupt retrieval (MRR = 0.0003).

!pip install sentence-transformers faiss-cpu xgboost datasets matplotlib -q

# ================================================================
# RAG ANTI-POISONING PIPELINE — v10 (FINAL)
# ================================================================
...
metrics_D = compute_metrics_batch(
    [r["results"] if r["status"]=="answered" else [] for r in resps_D], EVAL_TARGETS)
